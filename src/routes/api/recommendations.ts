...
import { validateRecommendationsParams } from "../../utils/recommendationsValidation";
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const validation = validateRecommendationsParams(req.query);

      if (!validation.valid || !validation.params) {
        return res.status(400).json({ error: validation.error });
      }

      // Fetch recommendations with automatic token refresh
      const recommendations = await handleTokenRefresh(
        spotifyId || null,
        accessToken,
        (token) => getRecommendations(token, validation.params!)
      );

      // Fetch audio features for all recommended tracks
      ...
      const transformedTracks: SpotifyTrack[] = recommendations.tracks.map(
      ...
      handleRouteError(res, err, "/api/recommendations");
    }
  }
);
